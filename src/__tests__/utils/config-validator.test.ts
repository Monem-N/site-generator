...
describe('Config Validator', () => {
  ...
  beforeEach(() => {
    ...
    (fs.accessSync as jest.Mock).mockImplementation((path: string, mode: number) => {
      return undefined;
    });
  ...
  test('should throw error for missing projectName', () => {
    ...
    expect(() => {
      validateConfig(invalidConfig as WebsiteGeneratorConfig);
  ...
  test('should validate configuration with plugins', () => {
    const configWithPlugins = {
      ...
      plugins: [
        { name: 'plugin1' },
        { name: 'plugin2', options: { option1: 'value1' } },
      ],
    };
    
    expect(() => {
      validateConfig(configWithPlugins as WebsiteGeneratorConfig);
    }).not.toThrow();
  });
});
