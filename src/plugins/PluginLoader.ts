...
export class PluginLoader {
   ...
   */
  loadPlugin(filePath: string, customOptions: Record<string, any> = {}): Plugin {
    try {
   ...
   */
  loadPlugins(dirPath: string, customOptions: Record<string, any> = {}): Plugin[] {
    try {
      // Check if the directory exists
      if (!fs.existsSync(dirPath)) {
        throw new FileSystemError(
          `Plugin directory not found: ${dirPath}`,
          { dirPath }
        );
      }
  ...
  private isValidPlugin(plugin: any): plugin is Plugin {
    return (
      ...
      typeof plugin.processContent === 'function' &&
      typeof plugin.processHtml === 'function' &&
      typeof plugin.getAssets === 'function'
    );
  }
}
