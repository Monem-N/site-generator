...
export class NavigationGenerator {
  ...
  private shouldIgnore(name: string, relativePath: string): boolean {
    ...
    return this.ignorePaths.some(ignorePath => 
      relativePath.split(path.sep).includes(ignorePath)
    );
  }
  ...
  private async buildNavigation(structure: any): Promise<NavigationItem[]> {
    ...
    const readmeFile = structure.files.find((file: any) => 
    ...
    for (const dir of structure.directories) {
      ...
      if (subNav.length > 0) {
        ...
        const dirReadme = dir.files.find((file: any) => 
          file.name.toLowerCase() === 'readme.md'
        );
        ...
        if (dirReadme) {
          title = await this.extractTitle(path.join(this.sourceDir, dir.path, dirReadme.name)) || title;
