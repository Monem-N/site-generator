...
"""
This script fixes unused variables in test files by prefixing them with underscores.
It focuses specifically on test files where most of the remaining issues are.
"""
...
# Regex patterns for finding variable declarations
VARIABLE_PATTERN = re.compile(r'(?:const|let|var)\s+([a-zA-Z0-9_]+)')
FUNCTION_PARAM_PATTERN = re.compile(r'\(([^)]*)\)')
ARROW_FUNCTION_PATTERN = re.compile(r'(?:const|let|var)\s+([a-zA-Z0-9_]+)\s*=\s*\([^)]*\)\s*=>')
FUNCTION_PATTERN = re.compile(r'function\s+([a-zA-Z0-9_]+)\s*\(')
DESTRUCTURING_PATTERN = re.compile(r'(?:const|let|var)\s+\{([^}]*)\}')
...
def main():
    """Main function to fix unused variables in test files."""
    print("Getting ESLint issues...")
    issues = get_eslint_issues()

    if not issues:
        print("No unused variable issues found in test files.")
        return

    print(f"Found {len(issues)} unused variable issues in test files.")
