...
cat > src/__tests__/utils/performance.test.ts.fixed << 'EOL'
...
describe('PerformanceMonitor', () => {
  ...
  test('should handle nested operations', () => {
    ...
    Date.now = jest.fn().mockImplementation(() => {
      ...
      switch (callCount) {
        case 1: return 1000; // parent start
        case 2: return 2000; // child start
        case 3: return 3000; // child end
        case 4: return 4000; // parent end
        default: return 0;
      }
    });
...
mv src/__tests__/utils/performance.test.ts.fixed src/__tests__/utils/performance.test.ts

echo "Decorator fixes completed!"
