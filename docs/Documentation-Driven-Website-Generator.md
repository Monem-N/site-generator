...
export class DesignSystemAdapter {
  ...
  private initializeTailwind(): void {
    ...
    this.styleMap = {
      ...
      'table-header':
        'bg-gray-100 border-b px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider',
  ...
  public getImportStatement(): string {
    switch (this.designSystem) {
      case 'material-ui':
        return `import { ${Array.from(this.componentMap.values()).join(
          ', '
